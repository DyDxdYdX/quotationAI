...
interface CreateInvoiceProps {
    ...
    prefilled_items?: Item[];
}

export default function CreateInvoice({ clients, prefilled_data = {}, prefilled_items = [] }: CreateInvoiceProps) {
    const [form, setForm] = useState({
        client_id: prefilled_data.client_id ? prefilled_data.client_id.toString() : '',
        quotation_id: prefilled_data.quotation_id ? prefilled_data.quotation_id.toString() : '',
        invoice_date: new Date().toISOString().split('T')[0],
        due_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Default 30 days
        status: 'pending',
        currency: 'RM',
        notes: '',
        items: prefilled_items.length > 0 ? prefilled_items : [{ description: '', quantity: 1, unit_price: 0, amount: 0 }],
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post('/invoices', form as any, {
            ...
            <div className="px-6 py-4">
                <div className="mb-6 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">Fill in the details below to create a new invoice</p>
                    </div>
                </div>
                        ...
                        <CardContent className="p-6">
                            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                <div className="space-y-1">
                                    <h4 className="text-lg font-bold text-foreground">Create Invoice</h4>
                                    <p className="text-sm text-muted-foreground">Review the details before creating the invoice</p>
                                </div>
                                <div className="flex gap-4">
                                    <Button
                                        type="button"
                                        variant="outline"
                                        onClick={() => router.visit('/manage-invoices')}
